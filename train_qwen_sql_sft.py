...
LR_SCHEDULER_TYPE (default ``cosine``), WARMUP_RATIO (default ``0.03``).
...
def _bootstrap_hf_packages() -> None:
    ...
    print(
        "[train_qwen_sql_sft] VERTEX_UPGRADE_TORCH=0: upgrading HF libs only (pinned to existing torch). "
...
def main() -> None:
    ...
    lr_scheduler_type = (os.environ.get("LR_SCHEDULER_TYPE") or "cosine").strip() or "cosine"
